...
locals {
  ...
  # DZ: add unique suffix at the end of cluster name and change region to 'us-east-2'
  name = "${local.name1}-dz"
  region = "us-east-2"
  # DZ end hack
...
module "eks_blueprints" {
  ...
  cluster_version = "1.24" # DZ: latest supported K8s API version
